...
int pointMap(int side, int i, int j)
{
  ...
  case 1: //[i][j][0] bottom face
    r = num_points_per_line * num_points_per_line * i + num_points_per_line * j;
    break;
  case 6: //[i][j][7] top face
    r = num_points_per_line * num_points_per_line * i + num_points_per_line * j + num_points_per_line - 1;
    break;
  case 2: //[i][0][j] front face
    r = num_points_per_line * num_points_per_line * i + j;
    break;
  case 5: //[i][7][j] back face
    r = num_points_per_line * num_points_per_line * i + (num_points_per_line * (num_points_per_line - 1)) + j;
    break;
  case 3: //[0][i][j] left face
    r = (num_points_per_line + 1) * i + j;
    break;
  case 4: //[7][i][j] right face
    r = ((num_points_per_line - 1) * num_points_per_line * num_points_per_line) + num_points_per_line * i + j;
    break;
  }
...
void showCube(struct world * jello)
{
  ...
  struct point normal[num_points_per_line][num_points_per_line];
  int counter[num_points_per_line][num_points_per_line];
  ...
  #define PROCESS_NEIGHBOUR(di,dj,dk) \
    ...
    kp=k+(dk);\
    if\
    (!( (ip>num_points_per_line - 1) || (ip<0) ||\
      (jp>num_points_per_line - 1) || (jp<0) ||\
    (kp>num_points_per_line - 1) || (kp<0) ) && ((i==0) || (i==num_points_per_line-1) || (j==0) || (j==num_points_per_line-1) || (k==0) || (k==num_points_per_line-1))\
       && ((ip==0) || (ip==num_points_per_line-1) || (jp==0) || (jp==num_points_per_line-1) || (kp==0) || (kp==num_points_per_line-1))) \
    {\
  ...
  if (viewingMode==0) // render wireframe
  {
    ...
    for (i=0; i<num_points_per_line; i++)
      for (j=0; j<num_points_per_line; j++)
        for (k=0; k<num_points_per_line; k++)
        {
          if (i*j*k*(num_points_per_line-1-i)*(num_points_per_line-1-j)*(num_points_per_line-1-k) != 0) // not surface point
            continue;
  ...
  else
  {
      ...
      // 1 = bottom, 2 = front, 3 = left, 4 = right, 5 = far, 6 = top
    {
      ...
      for (i=0; i <num_points_per_line; i++) // reset buffers
        for (j=0; j <num_points_per_line ; j++)
        {
      ...
      for (i=0; i <= num_points_per_line-2; i++)
        for (j=0; j <= num_points_per_line-2; j++) // process block (i,j)
        {
        ...
        for (j=1; j<=num_points_per_line-1; j++) 
        {
